...
fn tag_by_name_from_anywhere() {
    ...
    env.cmd()
        ...
        .args(["-tag", "add", "--workspace", "proj", "rust"])
        .assert()
        .success();
    env.cmd()
        .current_dir(env.home.path())
        .args(["-tag", "list", "--workspace", "proj"])
        .assert()
        .success()
        .stdout(predicate::str::contains("rust"));
}
...
fn archive_hides_from_list_until_flagged() {
    ...
    env.cmd().args(["-list"]).assert().success().stdout(predicate::str::contains("old\t"));
}
...
fn tag_with_unknown_workspace_name_errors() {
    let env = Env::new();
    env.cmd()
        .args(["-tag", "add", "--workspace", "ghost", "rust"])
        .assert()
        .failure()
        .stderr(predicate::str::contains("no such workspace"));
}
